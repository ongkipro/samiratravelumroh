...
export function FloatingWhatsApp() {
  ...
  useEffect(() => {
    ...
    const updateVisibility = () => {
      // Hide while user is viewing the Hero section (within initial viewport or 450px)
      const heroThreshold = Math.min(window.innerHeight * 0.65, 480);
      const isPastHero = window.scrollY > heroThreshold;
      setIsVisible(isPastHero && !footerInView);
    };

    if (footerEl) {
      observer = new IntersectionObserver(
        (entries) => {
          footerInView = entries[0]?.isIntersecting ?? false;
          updateVisibility();
        },
    ...
    return () => {
      ...
      if (observer && footerEl) observer.unobserve(footerEl);
    };
  }, []);
  ...
  return (
    <aside
      aria-label="Layanan Konsultasi WhatsApp"
    ...
    >
      ...
      <div className="relative inline-flex">
        ...
        <button
          ...
          className="relative inline-flex items-center gap-2.5 px-4 py-3.5 rounded-full bg-[#15803D] hover:bg-[#166534] text-white font-bold shadow-lg shadow-emerald-950/25 hover:shadow-xl hover:scale-105 active:scale-95 transition-all text-sm cursor-pointer"
