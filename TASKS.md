...
- [ ] **Milestone 1**: Project Scaffolding, Modern Tooling & Sacred Design Tokens (Phase 1)
- [ ] **Milestone 2**: Decoupled Data Access Layer (DAL) & TypeScript Models (Phase 2)
- [ ] **Milestone 3**: Global Shell — Modern Header, Mega-Menu, Cmd+K Global Search & Mobile App Dock (Phase 3)
- [ ] **Milestone 4**: Master Homepage ("The Open Sanctuary") & Interactive Client Islands (Phase 4)
- [ ] **Milestone 5**: Programmatic City Landing Pages (11 Embarkasi) & Dual-Mode Flight Tables (Phase 5)
- [ ] **Milestone 6**: Umroh Plus Suite (5 Destinasi) & Haji Khusus Furoda USD 17.000 (Phase 6)
- [ ] **Milestone 7**: Direktori 26 Kantor Cabang Fisik & Peluang Kemitraan DGi (Phase 7)
- [ ] **Milestone 8**: Kredensial Otoritas, Rekor MURI, Guinness World Records & Testimoni Medis (Phase 8)
- [ ] **Milestone 9**: Inbound Content Silo (400 Artikel Terindeks + In-Article WhatsApp Callout) (Phase 9)
- [ ] **Milestone 10**: SEO Engine Otomatis, Dynamic Sitemap (456 URLs) & JSON-LD (Phase 10)
- [ ] **Milestone 11**: Audit Aksesibilitas, Mobile Anti-Zoom, Core Web Vitals & Production Build (Phase 11)
...
- [ ] **Task 1.1**: Inisialisasi konfigurasi proyek: `package.json`, `tsconfig.json`, `next.config.ts`, `postcss.config.mjs`, dan `.gitignore`.
- [ ] **Task 1.2**: Instalasi dependensi esensial via `pnpm`:
...
- [ ] **Task 1.3**: Konfigurasi Tailwind CSS 4 & CSS Variables resmi *The Sacred Palette (Madina Tone)* di `src/app/globals.css`:
...
- [ ] **Task 1.4**: Konfigurasi font modern di `src/app/layout.tsx`:
...
- [ ] **Task 1.5**: Konfigurasi Mobile Viewport & Aturan Anti-Zoom di `src/app/layout.tsx`:
...
- [ ] **Task 2.1**: Definisikan tipe data type-safe di `src/types/`:
...
- [ ] **Task 2.2**: Bangun data store lokal di `src/data/tours.ts`:
...
- [ ] **Task 2.7**: Bangun utility helper di `src/lib/`:
...
- [ ] **Task 4.1**: Bangun `src/components/home/HeroSection.tsx`:
  - Layout asimetris split editorial dengan negative space lega.
  - Latar belakang foto fajar Ka'bah resolusi tinggi 16:9 (`banner-hero-kaaba-panorama.jpg`) dengan gradient overlay halus.
...
- [ ] **Task 4.2**: Bangun `src/components/home/CertaintyBar.tsx`:
...
- [ ] **Task 4.5**: Bangun `src/components/home/CompetitiveAdvantageSection.tsx`:
  - 4 pilar keunggulan: Skala Charter Flight, 26 Cabang Fisik, Teknologi Audio Receiver (APS), Kesiapan Layanan Medis Cuci Darah.
- [ ] **Task 4.6**: Bangun `src/components/home/FinancingCalculatorIsland.tsx` (Client Island):
  - Slider interaktif DP dan pilihan tenor cicilan syariah dengan kalkulasi instan.
- [ ] **Task 4.7**: Bangun `src/components/home/BranchQuickSelector.tsx` & `src/components/home/RecentArticlesSection.tsx`.

### Phase 5: Programmatic City Landing Pages (11 Embarkasi) & Dual-Mode Tables
- [ ] **Task 5.1**: Bangun halaman induk katalog `/paket-umroh` (`src/app/paket-umroh/page.tsx`).
- [ ] **Task 5.2**: Bangun dynamic route `src/app/paket-umroh/[city]/page.tsx` dengan `generateStaticParams()` untuk 11 kota:
  - Jakarta, Surabaya, Medan, Makassar, Palembang, Padang, Pontianak, Aceh, Denpasar, Batam, Pekanbaru.
- [ ] **Task 5.3**: Bangun komponen `DualModeTable.tsx`:
...
- [ ] **Task 5.4**: Integrasikan brosur fisik A4 (`brosur-keberangkatan-[xx].webp`) dengan tombol unduh.
- [ ] **Task 5.5**: Sematkan Schema.org `TouristTrip`, `Product`, dan `LocalBusiness`.

### Phase 6: Umroh Plus Suite (5 Destinasi) & Haji Khusus Furoda
- [ ] **Task 6.1**: Bangun halaman katalog `/umroh-plus` (`src/app/umroh-plus/page.tsx`).
- [ ] **Task 6.2**: Bangun dynamic route `src/app/umroh-plus/[slug]/page.tsx` untuk 5 rute:
...
- [ ] **Task 6.3**: Bangun landing page high-ticket `/haji-khusus-furoda` (`src/app/haji-khusus-furoda/page.tsx`):
...
- [ ] **Task 7.1**: Bangun halaman direktori `/kantor-cabang` (`src/app/kantor-cabang/page.tsx`) dengan filter 5 kepulauan.
- [ ] **Task 7.2**: Bangun dynamic route `src/app/kantor-cabang/[slug]/page.tsx` untuk 26 kantor cabang resmi:
  - Alamat fisik lengkap, sematan Google Maps, jam operasional, dan tombol WhatsApp langsung ke nomor cabang.
- [ ] **Task 7.3**: Bangun landing page `/kemitraan-dgi` (`src/app/kemitraan-dgi/page.tsx`):
...
- [ ] **Task 8.1**: Bangun halaman `/tentang-kami` (`src/app/tentang-kami/page.tsx`):
  - Sejarah perusahaan, profil founder, legalitas SK Kemenag, rekor dunia Guinness, 3 rekor MURI, dan 4 rekening resmi.
- [ ] **Task 8.2**: Bangun halaman `/testimoni` (`src/app/testimoni/page.tsx`):
...
- [ ] **Task 9.1**: Bangun direktori artikel `/artikel` (`src/app/artikel/page.tsx`) dengan pencarian instan dan filter kategori.
- [ ] **Task 9.2**: Bangun dynamic route `src/app/artikel/[slug]/page.tsx` dengan `generateStaticParams()` yang meng-generate **400 artikel secara statis**:
...
- [ ] **Task 10.1**: Implementasi `generateMetadata()` pada setiap rute statis dan dinamis dengan OpenGraph cards.
- [ ] **Task 10.2**: Bangun `src/app/sitemap.ts` dinamis yang mengindeks **456 URL** secara otomatis.
- [ ] **Task 10.3**: Bangun `src/app/robots.ts` ramah crawler mesin pencari.
- [ ] **Task 10.4**: Validasi seluruh struktur Schema.org JSON-LD.

### Phase 11: Audit Aksesibilitas, Mobile Anti-Zoom & Production Build
- [ ] **Task 11.1**: Audit bebas auto-zoom (seluruh input mobile `>= 16px`).
- [ ] **Task 11.2**: Audit touch targets minimal 44px × 44px pada layar sentuh.
- [ ] **Task 11.3**: Uji coba end-to-end pesan pembuka WhatsApp pada seluruh link konversi.
- [ ] **Task 11.4**: Uji coba kompilasi TypeScript (`npx tsc --noEmit`) = 0 error.
- [ ] **Task 11.5**: Uji coba kompilasi produksi (`npm run build`) = 456 rute SSG sukses tanpa error.
- [ ] **Task 11.6**: Audit Google Lighthouse & Core Web Vitals (Target: Mobile > 92, Desktop > 98).
